        ...
        for (const file of filenames) {
          try {
            ...
            // skip index files
              ...
              continue
            } else if (file.endsWith['-development.js'] && process.env.NODE_ENV !== 'development')
              // ignore development handlers if not in development
              continue
            else {
              const name = file.replace(/\.js$/, '')
              if (name !== name.toLowerCase()) throw new Error('fetchHandlers found non lowercase name:', name)
              const handler = require(filePath + file).default
              if (typeof handler !== type) {
                throw new Error(`fetchHandlers ${name} (${file}) is not a ${type}`)
              }
              if (handlers[name])
                logger.warn('fetchHandlers: handler', name, 'is being replaced from directory', dirPath)
